console.log = () => {};
const CHAT_PREFIX = "chat_"; // Prefix for identifying chat sessions in localStorage// Track active streaming state
...
async function fetchModels() {
  try {
    ...
    if (data && Array.isArray(data.models)) {
      ...
      console.log("Invalid models data format:", data);
...
function updateModelCheckboxes() {
  console.log(
    "Updating model checkboxes with",
    availableModels.length,
    "models"
  );
  // First, find the container in the settings modal
  const modelCheckboxesContainer = document.querySelector(
    "#models-dropdown-container"
  );
  if (!modelCheckboxesContainer) {
    console.log("Model checkboxes container not found");
    return;
  }
  ...
  const modelsDropdownMenu = modelCheckboxesContainer.querySelector(
    ".model-dropdown-menu"
  );
  if (!modelsDropdownMenu) {
    console.log("Models dropdown menu not found");
    return;
  }
...
function updateSelectedModelsDisplay() {
  ...
  const selectedModelsDisplay = document.getElementById(
  ...
  if (selectedModels.length === 0) {
    ...
    selectedModelsDisplay.textContent = `${selectedModels[0]}, ${
      selectedModels[1]
...
function updateStreamingMessage(elements, text, modelInfo = {}) {
  // 1) Update main content
  if (elements.content) {
    elements.content.innerHTML = markdownToHtml(text || "");
  }

  // 2) Update header info (model & token count)
  if (elements.modelInfo) {
    ...
    const name = modelInfo.model || modelInfo.model_name;
    const toks = modelInfo.tokens ?? modelInfo.token_count;
    if (modelNameEl && name) {
      modelNameEl.textContent = name;
    }
    if (tokenCountEl && typeof toks === "number") {
      tokenCountEl.textContent = `${toks} tokens`;
    }
  }

  // 3) Also mirror into best-model panel (so the "current best" updates as you stream)
...
function updateStreamingProgress(elements, progress, modelStats = null) {
  ...
  if (modelStats && elements.stats) {
    ...
    for (const [model, stats] of Object.entries(modelStats)) {
      statsHtml += `<div><strong>${model}:</strong> Score: ${stats.score.toFixed(
        2
      )} | ${stats.done ? "✓" : "..."}</div>`;
    }
...
function markdownToHtml(markdown) {
  ...
  if (window.marked && typeof marked.parse === "function") {
    try {
      ...
      console.log(
        "marked.parse failed, falling back to simple newline replacement:",
        e
      );
    ...
    // Detect closing bracket (only if not escaped)
      ...
      if (bracketCount === 0 && startIndex !== -1) {
        ...
        try {
          ...
          console.log("Invalid JSON found:", jsonStr.substring(0, 100) + "...");
        }
        startIndex = -1;
      }
    }
...
document
  ...
  .addEventListener("click", function () {
    ...
    if (modelCheckboxes.length > 0) {
      ...
      if (
        document.getElementById("modelLlama") &&
        document.getElementById("modelLlama").checked
      ) {
        activeModels.push("llama3.1");
        modelSettings.llama = true;
      }
      if (
        document.getElementById("modelMistral") &&
        document.getElementById("modelMistral").checked
      ) {
        activeModels.push("mistral");
        modelSettings.mistral = true;
      }
      if (
        document.getElementById("modelQwen") &&
        document.getElementById("modelQwen").checked
      ) {
    ...
    const algorithmType = document.querySelector(
      'input[name="algorithmType"]:checked'
    ).value;
    ...
    if (algorithmType === "stepwise") {
      ...
      const alphaWeight = document.getElementById("alphaWeight")
        ? parseFloat(document.getElementById("alphaWeight").value)
        : 0.7;
      const betaWeight = document.getElementById("betaWeight")
        ? parseFloat(document.getElementById("betaWeight").value)
        : 0.3;
      ...
      llmMsConfig = {
        ...
        MODELS: activeModels,
        START_TOKENS: parseInt(
          document.getElementById("tokenAllocation").value
        ),
        MAX_ROUNDS: 10,
        MAX_TOKENS: parseInt(document.getElementById("tokenAllocation").value),
        ALPHA: settings.weights.alpha,
        BETA: settings.weights.beta,
        DYNAMIC_MARGIN_COEFF:
          calculatedMarginRatio ||
          parseFloat(document.getElementById("earlyStoppingMargin").value) ||
          0.5,
...
function initAdvancedSettingsModal() {
  ...
  const dynamicMarginContainer = document.getElementById(
  ...
  const maxRoundsLabel = maxRoundsField
    ? maxRoundsField.previousElementSibling
    : null;
  ...
  if (algorithmStepwise) {
    algorithmStepwise.addEventListener("change", function () {
      if (this.checked) {
        ...
        if (dynamicMarginContainer)
          dynamicMarginContainer.classList.remove("d-none");
        if (startTokensField)
          startTokensField.parentElement.classList.remove("d-none");
  ...
  if (algorithmMAB) {
    algorithmMAB.addEventListener("change", function () {
      if (this.checked) {
        if (mabSettingsContainer)
          mabSettingsContainer.classList.remove("d-none");
        if (dynamicMarginContainer)
          dynamicMarginContainer.classList.add("d-none");
        if (startTokensField)
          startTokensField.parentElement.classList.add("d-none");

        if (maxRoundsField) {
          maxRoundsField.parentElement.classList.add("d-none");
        }
...
document
  ...
  ?.addEventListener("click", function () {
    ...
    const tokenAllocation = parseInt(
      document.getElementById("tokenAllocation").value
    );
    ...
    if (algorithmType === "stepwise") {
      ...
      settings.startTokens = parseInt(
        document.getElementById("tokenAllocation").value
      );
      settings.maxRounds = 10;
      settings.weights.alpha = parseFloat(
        document.getElementById("alphaWeight").value
      );
      settings.weights.beta = parseFloat(
        document.getElementById("betaWeight").value
      );
      settings.earlyStoppingParams.dynamicMarginCoeff = parseFloat(
        document.getElementById("dynamicMarginCoeff").value
      );
      // Update llmMsConfig for Stepwise/OUA algorithm
      llmMsConfig = {
      ...
      const explorationCoeff =
        parseFloat(document.getElementById("maxRounds").value) ||
        parseFloat(document.getElementById("xploreCoeff").value) ||
        0.3;
    ...
    const algorithmIndicator =
      settings.algorithmType === "stepwise" ? "Stepwise" : "MAB";
    document.getElementById(
      "modelbutton"
    ).textContent = `LLM-MS-OUA (${algorithmIndicator})`;
    // Close the modal
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("advancedSettingsModal")
    );
    if (modal) modal.hide();
    // Show notification
    console.log(
      `Advanced LLM-MS settings applied successfully\nAlgorithm: ${algorithmIndicator}`
    );
  });
...
function updateModelDropdown(data) {
  ...
  if (defaultModel) {
    ...
    dropdownButton.setAttribute(
      "aria-label",
      `Selected model: ${defaultModel.name}`
    );
...
function saveChatMemory(newHiddenHistory = null) {
  ...
  const existingSessionData =
    JSON.parse(localStorage.getItem(CHAT_PREFIX + sessionName)) || {};
  // If hiddenHistory is not set in the existing session, use the provided parameter
  const updatedHiddenHistory =
    newHiddenHistory !== null
      ? newHiddenHistory
      : existingSessionData.hiddenHistory || [];
  const sessionData = {
    history: chatHistory,
...
async function manageChatHistory() {
  ...
  const modelContextLimits = JSON.parse(
    localStorage.getItem("modelContextLimits") || "{}"
  );
  ...
  const hiddenHistoryMessage =
    sessionData.hiddenHistory &&
    Object.keys(sessionData.hiddenHistory).length > 0
      ? sessionData.hiddenHistory
      : null;
  const histodata = {
    model: selectedModel,
    messages: hiddenHistoryMessage
      ? [hiddenHistoryMessage, ...messagesFromLastSummarized]
      : messagesFromLastSummarized,
  ...
  try {
    ...
    console.log("Error managing chat history:", error);
  }
}
...
function appendLastMessage() {
  ...
  bubbleDiv.className =
    "flex flex-col items-start text-left " + (innerWidth < 380 ? "p-1" : "p-2");
  // Wrap the parsed content in a div with left padding to simulate a tab
  const indentedContent = `<div style="padding-left: 2em;">${marked.parse(
    lastMessage.content
  )}</div>`;
...
function showErrorModal(
  errorTitle = "Unable to connect to Ollama",
  errorMessage
) {
...
async function assignSessionNameBySystem(messages) {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to generate session name: ${response.statusText}`
      );
    }
    ...
    if (summary) {
      ...
      if (!rawSessionData) {
        console.log("No data in localStorage for session:", sessionName);
        return;
      }
    ...
    console.log("Error assigning session name:", error);
...
document
  .getElementById("clear-all-chats")
...
async function submitRequest() {
  ...
  const totalTokens =
    messagesFromLastSummarized.reduce((count, message) => {
      return count + countTokens(message?.content);
    }, 0) +
    (Array.isArray(hiddenHistory)
      ? hiddenHistory.reduce((count, message) => {
          return count + countTokens(message?.content);
        }, 0)
      : 0);
  ...
  if (totalTokens > maxContextTokens) {
    console.log(
      "submitRequest: Total tokens exceed limit. Managing chat history..."
    );
  ...
  try {
    ...
    console.log("submitRequest: Error during submission:", error);
...
if (llmmstoggle) {
  ...
  llmmstoggle.addEventListener("click", () => {
    if (llmmstoggle.checked) {
      ...
      const algorithmType =
        llmMsConfig.ALGORITHM_TYPE === "stepwise" ? "OUA" : "MAB";
      ...
      if (
        savedModel &&
        (savedModel === "LLM-MS-OUA" || savedModel === "LLM-MS-MAB")
      ) {
...
function updateModelResponse(
  ...
  if (!modelCard) {
    ...
    console.log("Creating model card for", modelName);
    ...
    modelCard.innerHTML = `
      ...
      <div class="model-metrics-container"></div>
    ...
    const modelsContainer =
      elements.container.querySelector(".models-container");
    if (modelsContainer) {
      modelsContainer.style.display = "block";

      modelsContainer.addEventListener("click", function (e) {
        const modelsScrollContainer = modelsContainer.querySelector(
          ".models-scroll-container"
        );
        if (modelsScrollContainer) {
          ...
          const titleElement = modelsContainer.querySelector(
            ".models-container-title"
          );
          if (titleElement) {
  ...
  if (!modelCard || modelCard.getAttribute("data-pruned")) {
    return;
  }
  // Update content if text is provided (optimized DOM update)
  if (text) {
  ...
  if (metrics && metrics.done === true && metrics.done == "stop") {
  ...
  if (metrics) {
    console.log("Updating metrics for model:", modelName);

    console.log("Metrics:", metrics);
    console.log("Model Card:", modelCard);
    ...
    if (metrics.score !== undefined) {
      ...
      // Don't return here - let the function continue with normal processing
    }
  }

  // Update error state if necessary
  if (isError) {
    console.log("Marking model card as error for", modelName);
    console.log("reason:", text, isError);
  ...
  if (isActive) {
    ...
    if (elements.bestModelContent) {
      ...
      if (currentContent) {
        ...
        //set active-model-card class only to best model
        Array.from(elements.modelsRow.querySelectorAll(".model-card")).forEach(
          (card) => {
      ...
      const bestModelContainer = elements.container.querySelector(
        ".best-model-response-container"
      );
      if (bestModelContainer) {
        bestModelContainer.style.display = "block";
        bestModelContainer.querySelector(
          ".best-model-title"
        ).textContent = `Best Model: ${modelName}`;
      }
    }
  }

  // Optimized scroll - only scroll if needed and use requestAnimationFrame
  if (isActive || metrics?.done) {
    requestAnimationFrame(() => scrollToBottom());
  }
}

function findAndUpdateBestModel(elements) {
  const allModelCards = elements.modelsRow.querySelectorAll(
    ".model-card[data-score]"
  );
  ...
  if (bestModelCard) {
    ...
    if (elements.bestModelContent) {
      ...
      if (bestModelContainer) {
        ...
        bestModelContainer.querySelector(
          ".best-model-title"
        ).textContent = `Best Model: ${bestModelName} (Score: ${highestScore.toFixed(
          3
        )})`;
      }
    }
  } else {
    console.warn("No models with scores found. Cannot determine best model.");
  }
}

function updateModelCardMetrics(modelCard, metrics) {
  console.log(
    "updateModelCardMetrics: Updating metrics for model card:",
    modelCard
  );
  if (!modelCard) {
    console.log("updateModelCardMetrics: Model card not found.");
    return;
  }
  ...
  if (!metricsBadge) {
    const header = modelCard.querySelector(".model-card-header .d-flex");
    metricsBadge = document.createElement("div");
    metricsBadge.className = "model-metrics-badge";
    header.appendChild(metricsBadge);
  }
  console.log(
    "updateModelCardMetrics: Metrics container and badge found:",
    metricsContainer,
    metricsBadge
  );
  if (!metricsContainer || !metricsBadge) return;

  // Check if model is pruned first - THIS IS WHERE YOU SET METRICS TO 0
  if (modelCard.getAttribute("data-pruned") === "true") {
    console.log(
      "updateModelCardMetrics: Model card is pruned, setting metrics to 0."
    );

    // Set score to 0 for pruned models
    modelCard.setAttribute("data-score", "0");

    // Update the badge to show 0 score
    metricsBadge.innerHTML = `<span class="score-badge">0</span>`;
    metricsBadge.classList.remove("d-none"); // Make sure it's visible

    console.log("updateModelCardMetrics: Pruned model metrics set to 0");
  }
  console.log(
    "updateModelCardMetrics: Metrics container and badge initialized."
  );
  let badgeHtml = "";
  ...
  console.log(
    "updateModelCardMetrics: Checking for score in metrics:",
    metrics,
    "Model Name:",
    modelName,
    "Has Score:",
  ...
  if (modelCard.getAttribute("data-pruned") === "true") {
    console.log(
      "updateModelCardMetrics: Model card is pruned, skipping metrics update."
    );
    //set to 0
    metricsBadge.innerHTML = `<span class="score-badge">0</span>`;
    metricsBadge.classList.add("d-none");
  }
  console.log("updateModelCardMetrics: Score extracted:", score);
  // Build the metrics display
  if (score !== null) {
    console.log(
      "updateModelCardMetrics: Score found for model:",
      modelName,
      "Score:",
      score
    );
  ...
  console.log("updateModelCardMetrics: Badge HTML constructed:", badgeHtml);
  ...
  console.log(
    "updateModelCardMetrics: Metrics badge updated with HTML:",
...
function finalizeStreamingMessage(elements, text, modelInfo = null) {
  ...
  if (elements.container) {
    ...
    const completedCards = elements.modelsRow.querySelectorAll(
      '.model-card[data-completed="true"]'
    );
    if (completedCards.length > 0) {
      ...
      if (elements.bestModelContent && finalModelContent) {
        ...
        if (bestModelContainer) {
          ...
          bestModelContainer.querySelector(
            ".best-model-title"
          ).textContent = `Best Model: ${finalModelName} (Completed)`;
        }
      }
...
// Markdown cache for better performance
const markdownCache = new Map();

function markdownToHtml(markdown) {
  // Return early for empty/null content
  if (!markdown) return "";
...
function scrollToBottom() {
  // Use more efficient scrolling - check element exists and user hasn't scrolled up
  const chatHistoryEl = document.getElementById("chat-history");
  if (chatHistoryEl && !userScrolledUp) {
...
function extractJsonObjects(buffer) {
  ...
  while (searchFrom < bufferLength) {
    ...
    // More robust JSON parsing that handles strings correctly
    ...
    if (endIndex !== -1) {
      // Extract JSON string efficiently
      jsons.push(buffer.slice(startIndex, endIndex + 1));
      searchFrom = endIndex + 1;
    } else {
      // No complete JSON found from this point
      break;
    }
  }
  ...
  return {
    objects: jsons,
    remainder: searchFrom > 0 ? buffer.slice(searchFrom) : buffer,
  };
}
...
async function sendMessage() {
  ...
  try {
    ...
    if (algorithmType === "stepwise") {
      ...
      configToSend.DYNAMIC_MARGIN_COEFF =
        llmMsConfig.DYNAMIC_MARGIN_COEFF || 0.5;
      configToSend.EMBEDDING_MODEL =
        llmMsConfig.EMBEDDING_MODEL || "nomic-embed-text";
    ...
    //create an element forach model response
    for (const model of configToSend.MODELS) {
      console.log("submitRequest: Initializing model response for:", model);
      updateModelResponse(activeMessageElements, model, "", false, false, 0);
    }
    // Ensure the scroll container is visible
    const modelsScrollContainer = activeMessageElements.container.querySelector(
      ".models-scroll-container"
    );
    if (modelsScrollContainer) {
      modelsScrollContainer.classList.remove("d-none");
    }

    try {
      // Added try-catch for the stream reading loop
      while (true) {
        const { done, value } = await reader.read();
        ...
        if (done) {
          ...
          if (jsonBuffer.trim().length > 0) {
            ...
            for (const jsonObjString of finalExtraction.objects) {
              try {
                ...
                if (data.status === "error" && activeMessageElements.content) {
                  console.log("Streaming error from server:", data.error);
                  updateModelResponse(
                  ...
                  activeMessageElements.content
                ) {
                  console.log(
                    "Final result received:",
                    data.message || data.completion || ""
                  );
                  finalizeStreamingMessage(
                    activeMessageElements,
                    markdownToHtml(data.message || data.completion || ""),
                    data.model_info || "Completed"
                  );
                } else if (activeMessageElements.content) {
                  // Handle other potential statuses
                  // Fallback for other data types if necessary
                  updateStreamingMessage(
                    activeMessageElements,
                    activeMessageElements.content.innerHTML +
                      markdownToHtml(data.message || JSON.stringify(data))
                  );
                }
                // Add more comprehensive status handling here if needed for final chunks
              } catch (e) {
                console.log(
                  "Error parsing JSON object from final buffer:",
                  e,
                  jsonObjString
                );
                if (activeMessageElements.content) {
                  console.log("Malformed data at stream end:", jsonObjString);

                  updateModelResponse(
                    activeMessageElements,
                    "Error",
                    `<p class="text-danger">Malformed data at stream end.</p>`,
                    true,
                    true
                  );
                }
            ...
            if (finalExtraction.remainder.trim().length > 0) {
              console.log(
                "Stream ended with non-JSON remainder:",
                finalExtraction.remainder
              );
              if (
                activeMessageElements.content &&
                !finalExtraction.objects.length
              ) {
                console.log(
                ...
                updateModelResponse(
                  activeMessageElements,
                  "Warning",
                  `<p class="text-warning">Stream ended with unprocessed data.</p>`,
                  true,
                  false
                );
              }
        ...
        // Process buffer incrementally to avoid re-processing
        while (jsonBuffer.length > 0) {
          const result = extractJsonObjects(jsonBuffer);

          if (result.objects.length === 0) {
            // No complete JSON objects found, keep remainder for next chunk
            break;
          }

          // Process all complete JSON objects found
          for (let i = 0; i < result.objects.length; i++) {
            const jsonObjString = result.objects[i];
            try {
              const data = JSON.parse(jsonObjString);
              ...
              if (data.status === "initialized") {
              ...
              } else if (data.status === "model_pruned") {
                if (activeMessageElements.container) {
                  console.log(
                    "Model pruned:",
                    data.model,
                  ...
                  // Mark the model as pruned
                  const modelCard =
                    activeMessageElements.modelsRow.querySelector(
                      `.model-card[data-model="${data.model}"]`
                    );
                  if (modelCard) {
                    modelCard.setAttribute("data-score", 0);
                    modelCard.setAttribute("data-pruned", "true");
                    modelCard.classList.add("pruned-model-card");
                    // MANUAL BADGE UPDATE - no function call needed
                    const metricsBadge = modelCard.querySelector(
                      ".model-metrics-badge"
                    );
                    if (metricsBadge) {
                      // Set the badge to show 0 score directly
                      metricsBadge.innerHTML = `<span class="score-badge">0</span>`;
                      metricsBadge.classList.remove("d-none"); // Make sure it's visible
                      console.log(
                        "Manually set badge to 0 for pruned model:",
                        data.model
                      );
                    } else {
                      // If badge doesn't exist, create it
                      const header = modelCard.querySelector(
                        ".model-card-header .d-flex"
                      );
                      if (header) {
                        const newBadge = document.createElement("div");
                        newBadge.className = "model-metrics-badge";
                        newBadge.innerHTML = `<span class="score-badge">0</span>`;
                        header.appendChild(newBadge);
                        console.log(
                          "Created new badge with score 0 for pruned model:",
                          data.model
                        );
                      }
                    }
                ...
                data.status === "model_progress" ||
                data.status === "model_update" ||
                data.status === "model_scored"
              ) {
                if (activeMessageElements.container) {
                  if (
                    data.status === "model_progress" ||
                    ...
                    partialout = markdownToHtml(data.partial_output);
                    console.log(
                      "Streaming progress for model:",
                      data.model,
                      "Partial Output:",
                      partialout
                    );
                  }
                  console.log("Streaming progress for model:", data.model);
                  // Update the model response with the current progress
                ...
                if (activeMessageElements.container) {
                  console.log(
                    "Streaming finished for model:",
                    data.model,
                    "Reason:",
                    data.reason
                  );
                  // Finalize the model response
                  // If the model is done, update the main content with the final result
                  updateModelResponse(
                    activeMessageElements,
                    data.model,
                    "",
                  ...
                  // console.log(`${data.model} finished. Reason: ${data.reason}`);
                }
              } else if (
                data.status === "partial_result" ||
                data.status === "interim_result"
              ) {
                if (activeMessageElements.content) {
                  // Main content update
                  console.log(
                ...
                console.log("Streaming error from server:", data.error);
                if (activeMessageElements.content) {
                  console.log("Error in streaming response:", data.error);
                  // Update the main content with the error message
                  updateModelResponse(
                    activeMessageElements,
                    "Error",
                    `<p class="text-danger">Server error: ${data.error}</p>`,
                    true,
                    true
                  );
                }
                ...
                if (activeMessageElements.content && data.message) {
                  console.log(
                    "Unknown data status, updating content with message:",
                    data.message
                  );
                  updateStreamingMessage(
                    activeMessageElements,
              ...
              console.log(
                "Error parsing JSON object from stream:",
                e,
                jsonObjString
              );
          ...
          // Immediately trim buffer to remainder, avoiding re-processing
          jsonBuffer = result.remainder;
        }
      }
    } catch (streamError) {
      // Catch errors from reader.read() or decoder
      console.log("Error reading or decoding stream:", streamError);
      isStreaming = false;
      if (activeController === controller) activeController = null;
      if (activeMessageElements.content) {
        console.log("Error in streaming response:", streamError.message);

        updateModelResponse(
          activeMessageElements,
          "Error",
          `<p class="text-danger">Stream connection error: ${streamError.message}</p>`,
          true,
          true
        );
      }
    ...
    if (error.name === "AbortError") {
      ...
      finalizeStreamingMessage(streamingElements, "Error: " + error.message, {
        model: "Error",
        tokens: 0,
      });
    }
