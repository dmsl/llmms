...
class BrowserRetriever {
    ...
    async retrieveRelevantDocuments(query, maxResults = 5) {
        ...
        try {
            ...
            const results = await this.vectorDB.findSimilar(Array.from(queryEmbedding), maxResults);
            console.log("retrieveRelevantDocuments: Retrieved", results.length, "documents");
