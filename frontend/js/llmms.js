...
async function sendMessage() {
    ...
    try {
        ...
        if (algorithmType === 'stepwise') {
            // OUA parameters
            configToSend.START_TOKENS = llmMsConfig.START_TOKENS || 1024;
        ...
        let buffer = '';
        let finalOutput = '';
        let finalModelInfo = { model: currentModel, tokens: 0 };
        // Track chunks for debugging if needed
        let chunkCount = 0;
        ...
        try {
            while (true) {
                ...
                buffer += chunk;
                // Log chunks for debugging (truncated to avoid console spam)
                console.debug(`Chunk ${chunkCount} received, size: ${chunk.length} bytes`);
                if (buffer.length > lastProcessedLength + 1000) {
                    console.debug(`Buffer getting large: ${buffer.length} bytes`);
                }
                // Extract complete JSON objects
                let jsonObjects = extractJsonObjects(buffer);
                ...
                for (const jsonStr of jsonObjects.objects) {
                    try {
                        ...
                        // Add debug info about received data
                        console.debug(`Processing ${data.status} update from ${algorithmType}`);

                        // Initialize model card for all models as soon as we know them
                        if (data.models && Array.isArray(data.models) && data.status === 'initialized') {
                            data.models.forEach(model => {
                                // Create a placeholder card for each model to ensure they exist
                                updateModelResponse(streamingElements, model, '', false, false);
                            });
                        }

                        // Enhanced metrics extraction
                        const extractAndUpdateMetrics = (data) => {
                            let metricsData = data.metrics || {};
                            // Add core metrics
                            if (data.round) metricsData.current_round = data.round;
                            if (data.cumulative_tokens) metricsData.cumulative_tokens = data.cumulative_tokens;
                            if (data.max_rounds) metricsData.max_rounds = data.max_rounds;

                            // Track model errors
                            if (!metricsData.model_errors) metricsData.model_errors = {};
                            if (data.status === 'model_error' && data.model && data.error) {
                                metricsData.model_errors[data.model] = data.error;
                            }

                            // Track model scores
                            if (data.status === 'model_scored' && data.model) {
                                if (!metricsData.model_scored_data) metricsData.model_scored_data = {};
                                metricsData.model_scored_data[data.model] = {
                                    score: data.score,
                                    q_similarity: data.q_similarity,
                                    inter_similarity: data.inter_similarity
                                };

                                // Update the model card with this specific model's metrics
                                const modelMetrics = {
                                    score: data.score,
                                    q_similarity: data.q_similarity,
                                    inter_similarity: data.inter_similarity
                                ...
                                updateModelResponse(
                                    ...
                                    data.model,
                                    data.partial_output || '',
                                    data.is_best_model || false,
                                    false, // isError
                                    modelMetrics
                                );
                            }

                            // Extract tokens per model if available
                            if (!metricsData.tokens_per_model) metricsData.tokens_per_model = {};
                            if (data.model && data.tokens) {
                                metricsData.tokens_per_model[data.model] = data.tokens;
                            }

                            // Extract metrics from all_models if present (stepwise)
                            if (data.all_models) {
                                const modelScores = {};
                                for (const [model, stats] of Object.entries(data.all_models)) {
                                    if (stats.score !== undefined) {
                                        modelScores[model] = stats.score;

                                        // Update specific model card
                                        updateModelResponse(
                                            streamingElements,
                                            model,
                                ...
                                metricsData.similarity_scores = modelScores;
                            }

                            // Extract metrics from model_stats if present (MAB)
                            if (data.model_stats) {
                                const modelRewards = {};
                                for (const [model, stats] of Object.entries(data.model_stats)) {
                                    if (stats.reward !== undefined) {
                                        modelRewards[model] = stats.reward;
                                ...
                                metricsData.rewards = modelRewards;
                            }
                        };

                        // Check for metrics and extract them
                        if (data.metrics) {
                            console.debug('Metrics received:', data.metrics);
                            extractAndUpdateMetrics(data);
                        } else {
                            extractAndUpdateMetrics(data);
                        }

                        // Handle errors and individual model progress
                        if (data.status === 'model_error') {
                            console.warn(`Model error: ${data.model} - ${data.error}`);
                            // Update model card with error state, but don't clear its content
                            const modelCard = streamingElements.container.querySelector(`.model-card[data-model="${data.model}"]`);
                            const existingContent = modelCard ? modelCard.querySelector('.model-card-content').innerHTML : '';
                        ...
                        if (algorithmType === 'mab') {
                            // Handle MAB-specific streaming updates
                            switch (data.status) {
                                case 'initialized':
                                    console.log("MAB initialized with models:", data.models);
                                    updateStreamingMessage(streamingElements, 'Initializing Multi-Armed Bandit approach...');
                                    break;
                                case 'initial_pull':
                                case 'model_finished':
                                case 'round_update':
                                    updateStreamingMessage(streamingElements,
                                        data.partial_output || 'Processing...',
                                        { model: `LLM-MS MAB (${data.model || data.chosen_model})`, tokens: data.total_tokens || data.tokens || 0 }
                                    );
                                    if (data.progress) {
                                        updateStreamingProgress(streamingElements, parseInt(data.progress), data.models_state);
                                    }
                                    if (data.models_outputs) {
                                        for (const [model, output] of Object.entries(data.models_outputs)) {
                                            if (output) {
                                                const isActive = (data.chosen_model && model === data.chosen_model) ||
                                                    (data.model && model === data.model);
                                                updateModelResponse(streamingElements, model, output, isActive);
                                            }
                                        }
                                    }
                                    // Extract MAB-specific metrics
                                    extractAndUpdateMetrics(data);
                                    break;
                                case 'early_stopping':
                                    updateStreamingMessage(streamingElements,
                                        'Early stopping triggered. Completing with best model...',
                                        { model: `LLM-MS MAB(${data.best_model})`, tokens: data.tokens || 0 }
                                    );
                                    if (data.best_model) {
                                        updateModelResponse(streamingElements, data.best_model, data.partial_output || '', true);
                                    }
                                    break;
                                case 'completing_best':
                                    updateStreamingMessage(streamingElements,
                                        data.partial_output || 'Completing...',
                                        { model: `LLM-MS MAB(${data.model})`, tokens: data.tokens || 0 }
                                    );
                                    if (data.model && data.partial_output) {
                                        updateModelResponse(streamingElements, data.model, data.partial_output, true);
                                    }
                                    break;
                                case 'final_result':
                                    updateStreamingMessage(streamingElements,
                                        data.output || 'No output provided.',
                                        { model: `LLM-MS MAB(${data.model})`, tokens: data.tokens || 0 }
                                    );
                                    if (data.model) {
                                        updateModelResponse(streamingElements, data.model, data.output || '', true);
                                    }
                                    break;
                                case 'error':
                                    updateStreamingMessage(streamingElements, `Error: ${data.error || 'Unknown error'}`);
                                    break;
                            }
                        } else {
                            // Handle OUA/Stepwise-specific streaming updates
                            switch (data.status) {
                                case 'initialized':
                                    updateStreamingMessage(streamingElements, 'Initializing OUA Allocation...');
                                    // Extract configuration info
                                    if (data.models) {
                                        streamingElements.container.setAttribute('data-models', data.models.join(','));
                                    }
                                    if (data.config) {
                                        const configStr = `Using ${data.config.models?.length || 0} models with α=${data.config.alpha}, β=${data.config.beta}`;
                                        updateStreamingMessage(streamingElements, configStr);
                                    }
                                    extractAndUpdateMetrics(data);
                                    break;
                                case 'round_start':
                                    ...
                                    break;
                                case 'model_progress':
                                    // Only update streaming message for best model
                                    if (data.is_best_model) {
                                        updateStreamingMessage(streamingElements,
                                            data.partial_output || 'Processing...',
                                            { model: `LLM-MS OUA(${data.model})`, tokens: data.tokens || 0 }
                                        );
                                    }

                                    // Always update individual model responses
                                    if (data.model && data.partial_output) {
                                        const isBest = data.is_best_model || false;
                                        // Remove <think> tags if present before displaying
                                        let cleanOutput = data.partial_output.replace(/<think>.*?<\/think>/g, '');
                                        updateModelResponse(streamingElements,
                                            data.model,
                                            cleanOutput,
                                            isBest,
                                            false, // isError
                                    ...
                                    // Extract stepwise-specific metrics
                                    extractAndUpdateMetrics(data);
                                    break;
                                case 'model_scored':
                                    // Update metrics with scoring information
                                    extractAndUpdateMetrics(data);
                                    break;
                                case 'round_summary':
                                    const progress = (data.round / llmMsConfig.MAX_ROUNDS) * 100;
                                    updateStreamingProgress(streamingElements, progress, data.all_models);
                                    if (data.model_outputs) {
                                        for (const [model, output] of Object.entries(data.model_outputs)) {
                                            const isBest = model === data.best_model;
                                            // Clean output before displaying
                                            let cleanOutput = output.replace(/<think>.*?<\/think>/g, '');
                                            updateModelResponse(streamingElements, model, cleanOutput, isBest);
                                        }
                                    }
                                ...
                                case 'early_stopping':
                                    updateStreamingMessage(streamingElements,
                                        data.output || `Early stopping triggered at round ${data.round || '?'}.`,
                                        { model: `LLM-MS OUA(${data.best_model})`, tokens: data.tokens || 0 }
                                    );
                                    if (data.best_model) {
                                        updateModelResponse(streamingElements, data.best_model, data.output || '', true);
                                    }
                                    extractAndUpdateMetrics(data);
                                    break;
                                case 'final_result':
                        ...
                        if (data.done) {
                            finalOutput = data.output || data.partial_output || '';
                            finalModelInfo = {
                            ...
                            if (data.final_metrics) {
                                updateModelCardMetrics(streamingElements.container.querySelector(`.model-card[data-model="${data.model}"]`), data.final_metrics);
                            }
                        }
                    } catch (e) {
                        console.error("Error parsing JSON:", e, "Raw JSON:", jsonStr);
                    }
                }
            }
            ...
            if (buffer.trim()) {
                try {
                    ...
                    try {
                        remainingData = JSON.parse(buffer);
                    } catch (e) {
                        try {
                            remainingData = JSON.parse(buffer + '}');
                        } catch (innerE) {
                            console.log("Could not parse remaining buffer:", buffer);
                        }
                    }
                    if (remainingData) {
                        if (algorithmType === 'mab') {
                            switch (remainingData.status) {
                                case 'initialized':
                                    updateStreamingMessage(streamingElements, 'Initializing Multi-Armed Bandit approach...');
                                    break;
                                case 'round_update':
                                    updateStreamingMessage(streamingElements,
                                        remainingData.partial_output || 'Processing...',
                                        { model: `LLM-MS MAB (${remainingData.chosen_model})`, tokens: remainingData.total_tokens }
                                    );
                                    if (remainingData.progress) {
                                        updateStreamingProgress(streamingElements, parseInt(remainingData.progress), remainingData.models_state);
                                    }
                                    if (remainingData.models_outputs) {
                                        for (const [model, output] of Object.entries(remainingData.models_outputs)) {
                                            if (output) {
                                                const isActive = model === remainingData.chosen_model;
                                                updateModelResponse(streamingElements, model, output, isActive);
                                            }
                                        }
                                    }
                                ...
                                case 'early_stopping':
                                    updateStreamingMessage(streamingElements,
                                        'Early stopping triggered. Completing with best model...',
                                        { model: `LLM-MS MAB(${remainingData.best_model})`, tokens: remainingData.tokens || 0 }
                                    );
                                    if (remainingData.best_model) {
                                        updateModelResponse(streamingElements, remainingData.best_model, remainingData.partial_output || '', true);
                                    }
                                    break;
                                case 'completing_best':
                                    updateStreamingMessage(streamingElements,
                                        remainingData.partial_output || 'Completing...',
                                        { model: `LLM-MS MAB(${remainingData.model})`, tokens: remainingData.tokens || 0 }
                                    );
                                    if (remainingData.model && remainingData.partial_output) {
                                        updateModelResponse(streamingElements, remainingData.model, remainingData.partial_output, true);
                                    }
                                    break;
                                case 'final_result':
                                    updateStreamingMessage(streamingElements,
                                        remainingData.output || 'No output provided.',
                                        { model: `LLM-MS MAB(${remainingData.model})`, tokens: remainingData.tokens || 0 }
                                    );
                                    if (remainingData.model) {
                                        updateModelResponse(streamingElements, remainingData.model, remainingData.output || '', true);
                                    }
                                    break;
                                case 'error':
                                    updateStreamingMessage(streamingElements, `Error: ${remainingData.error || 'Unknown error'}`);
                                    break;
                            }
                        } else {
                            switch (remainingData.status) {
                                case 'initialized':
                                    updateStreamingMessage(streamingElements, 'Initializing Stepwise Token Allocation...');
                                    break;
                                case 'round_start':
                                ...
                                case 'model_progress':
                                    updateStreamingMessage(streamingElements,
                                        remainingData.partial_output || 'Processing...',
                                        { model: `LLM-MS OUA(${remainingData.model})`, tokens: remainingData.tokens || 0 }
                                    );
                                    if (remainingData.model && remainingData.partial_output) {
                                        const isBest = remainingData.is_best_model || false;
                                        updateModelResponse(streamingElements, remainingData.model, remainingData.partial_output, isBest);
                                    }
                                    break;
                                case 'round_summary':
                                    const progress = (remainingData.round / llmMsConfig.MAX_ROUNDS) * 100;
                                    updateStreamingProgress(streamingElements, progress, remainingData.all_models);
                                    if (remainingData.model_outputs) {
                                        for (const [model, output] of Object.entries(remainingData.model_outputs)) {
                                            const isBest = model === remainingData.best_model;
                                            updateModelResponse(streamingElements, model, output, isBest);
                                        }
                                    }
                                    break;
                                case 'early_stopping':
                                    updateStreamingMessage(streamingElements,
                                        remainingData.output || 'Early stopping triggered.',
                                        { model: `LLM-MS OUA(${remainingData.best_model})`, tokens: remainingData.tokens || 0 }
                                    );
                                    if (remainingData.best_model) {
                                        updateModelResponse(streamingElements, remainingData.best_model, remainingData.output || '', true);
                                    }
                                    break;
                                case 'final_result':
                                    updateStreamingMessage(streamingElements,
                        ...
                        if (remainingData.done) {
                            finalOutput = remainingData.output || remainingData.partial_output || finalOutput;
                            finalModelInfo = {
                                model: remainingData.model || finalModelInfo.model,
                                tokens: remainingData.tokens || finalModelInfo.tokens
                            };
                        }
                    }
                } catch (e) {
                    console.log("Ignoring error in final buffer processing:", e);
                }
            }
        } catch (error) {
        ...
        finalizeStreamingMessage(streamingElements, finalOutput, finalModelInfo);
        // Update chat session storage
        console.log("submitRequest: Appending assistant response to history.");
        const assistantMessage = { role: 'assistant', content: finalOutput };
        chatHistory.push(assistantMessage);
        chatmodelhistory.push(finalModelInfo.model);
        saveChatMemory();
        selectedFile = null;
...
//belpw jere is script.js so we can use it in the new chat
