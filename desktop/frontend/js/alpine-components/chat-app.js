...
function chatApp() {
    return {
        ...
        async init() {
            ...
            // Set up MCP config modal
            if (window.desktop?.isElectron) {
                // Load initial config and tools
        ...
        async sendMessage() {
            ...
            try {
                ...
                const result = await window.askAgent(query, toolSchemas, this.selectedModel, conversationHistory, this.uploadedFile);
